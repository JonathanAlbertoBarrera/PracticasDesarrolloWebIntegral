  ...
  if (cocktails.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-neutral-200 bg-white px-8 py-16 text-center">
        <p className="text-sm text-neutral-500">No hay cócteles todavía.</p>
        <p className="mt-1 text-sm text-neutral-400">
          Usa el botón &quot;Nuevo cóctel&quot; para agregar uno.
        </p>
      </div>
    );
  }
      ...
      <table className="w-full text-left text-sm">
        <thead>
          <tr className="border-b border-neutral-100 bg-neutral-50/80">
            <th className="px-5 py-3 font-medium text-neutral-500">Cóctel</th>
          ...
          {cocktails.map((cocktail) => (
            <tr key={cocktail.id} className="transition hover:bg-neutral-50/60">
              <td className="px-5 py-4">
                <div>
                  <p className="font-medium text-neutral-900">{cocktail.name}</p>

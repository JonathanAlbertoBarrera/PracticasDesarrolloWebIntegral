...
//   runtime:    export default function PetsModule()
//   build-time: export function PetsModule()   ← named export de paquete
...
export function PetsModule() {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return pets;
    return pets.filter((p) => [p.name, p.species, p.breed].some((f) => f.toLowerCase().includes(q)));
  }, [pets, query]);

  const available = pets.filter((p) => p.status === 'available').length;
  const treatment = pets.filter((p) => p.status === 'adopted').length;
  ...
  const handleDelete = (p: Pet) => { if (confirm(`¿Eliminar a "${p.name}"?`)) petsStore.remove(p.id); };
      ...
      <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, gap: 12 }}>
        <div>
          ...
          <p style={{ margin: '4px 0 0', color: theme.colors.textMuted, fontSize: 14 }}>
            CRUD en memoria · paquete <code>@nimbus/pets-front</code> (compilado en el host)
          </p>
        </div>
            ...
            <tbody>
              {filtered.map((p) => (
                <tr key={p.id} style={{ borderTop: `1px solid ${theme.colors.border}` }}>
                  ...
                  <Td style={{ textAlign: 'right', whiteSpace: 'nowrap', display: 'none' }}>

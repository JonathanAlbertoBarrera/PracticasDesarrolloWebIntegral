  ...
  return (
    <aside style={{ width: 248, background: theme.colors.shellBg, color: theme.colors.textOnDark, padding: 20, display: 'flex', flexDirection: 'column', gap: 24, flexShrink: 0 }}>
        ...
        {items.map((it) => {
          ...
          return (
            ...
            >
              {it.label}
              {it.remote && (
                <span title="Cargado como microfront remoto" style={{ fontSize: 10, opacity: 0.7, border: '1px solid currentColor', borderRadius: 6, padding: '1px 5px' }}>
                  remote
                </span>
              )}
            </button>
      ...
      <div style={{ marginTop: 'auto', fontSize: 11, color: theme.colors.textOnDarkMuted, lineHeight: 1.6 }}>
        Demo de microfrontends<br />Module Federation · Vite
      </div>
    </aside>
  );
}